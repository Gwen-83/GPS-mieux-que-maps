...
# On s'assure que la colonne unique_name est bien présente
if 'unique_nm' in gdf.columns: # Shapefile tronque parfois les noms de colonnes à 10 caractères
    gdf['unique_name'] = gdf['unique_nm']
else:
    gdf['unique_name'] = gdf['name'].astype(str) + "_" + gdf['osm_id'].astype(str)

print("Calcul des adjacences via Voronoi...")
# On spécifie les IDs uniques pour que les voisins soient indexés correctement
weights = Voronoi.from_dataframe(gdf)

adjacences_dict = {}

for i, row in gdf.iterrows():
    # On utilise le NOM UNIQUE (Nom_ID)
    id_ville = row['unique_name']
    
    indices_voisins = weights.neighbors[i]
    
    # On récupère les IDs UNIQUES des voisins
    noms_voisins = gdf.iloc[indices_voisins]['unique_name'].tolist()
    
    adjacences_dict[id_ville] = noms_voisins

output_path = r"C:\Users\Gwénaël\OneDrive\Bureau\ENAC\Programmation\projet_GPS\src\data\adjacences_villes.json"
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(adjacences_dict, f, ensure_ascii=False, indent=4)
