...
PATH_OUTPUT = r"C:\Users\Gwénaël\OneDrive\Bureau\ENAC\Programmation\projet_GPS\src\data\routes_haute_precision.json"

SPEED_DEFAULTS = {
    'motorway': 130, 'motorway_link': 90, 'trunk': 110, 'trunk_link': 70,
    'primary': 80, 'primary_link': 50, 'secondary': 80, 'secondary_link': 50,
    'tertiary': 50, 'tertiary_link': 50, 'residential': 50, 'living_street': 20,
...
def build_projected_graph(gdf_roads_projected):
    ...
    # IMPORTANT: On inverse la géométrie pour que linemerge ne crée pas de sauts
    edges_reverse['geometry'] = edges_reverse['geometry'].apply(lambda x: LineString(list(x.coords)[::-1]))
    
    all_edges = pd.concat([edges_direct, edges_reverse], ignore_index=True)
    
    # 5. Création du MultiDiGraph (plus robuste pour les routes parallèles)
    G = nx.MultiDiGraph()
    for _, row in all_edges.iterrows():
        G.add_edge(row['u'], row['v'], weight=row['weight'], time=row['time'], geometry=row['geometry'])

    # Nettoyage : suppression des petites composantes isolées (souvent des impasses privées)
    if len(G) > 0:
...
cities_gdf = gpd.GeoDataFrame(cities_df, geometry=gpd.points_from_xy(cities_df.lon, cities_df.lat), crs="EPSG:4326").to_crs(epsg=2154)

processed_cities = {}
city_coords = np.c_[cities_gdf.geometry.x, cities_gdf.geometry.y]
dists, idxs = tree.query(city_coords, k=1) # On peut augmenter k pour chercher une route plus importante si besoin
...
for ville_nom, villes_voisines in tqdm(adj_data.items(), desc="Calcul"):
    if ville_nom not in processed_cities or processed_cities[ville_nom]['dist_to_road'] > 5000:
        continue

    start_info = processed_cities[ville_nom]
    final_output[ville_nom] = {"coords": start_info['orig_data'], "adjacents": []}

    for voisin_nom in villes_voisines:
        if voisin_nom not in processed_cities: continue
        
        end_info = processed_cities[voisin_nom]
        
        try:
            # Dijkstra
            path_nodes = nx.shortest_path(G, source=start_info['node'], target=end_info['node'], weight='weight')
            ...
            for u, v in zip(path_nodes[:-1], path_nodes[1:]):
                # Dans un MultiDiGraph, on prend l'arête la plus courte entre u et v
            ...
            # Conversion en GeoSeries pour reprojection massive
            full_line_wgs84 = gpd.GeoSeries([full_line_proj], crs="EPSG:2154").to_crs(epsg=4326).iloc[0]
            ...
            # Si la route est 10x plus longue que la distance à vol d'oiseau, c'est suspect
            if dist_km > 250: continue 

            final_output[ville_nom]["adjacents"].append({
                "nom": voisin_nom,
                "distance_km": round(dist_km, 3),
                "vitesse_moyenne_kmh": round((dist_km / (total_time / 3600)), 1) if total_time > 0 else 0,
